...
namespace MyQuantifyApp.Database
{
    public class SQLiteDataService
    {
        ...
        private static string GetDatabaseFilePath()
        {
            ...
            const string dbFileName = "quantify.sqlite";
            string appDataPath = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData);
            string specificPath = Path.Combine(appDataPath, appName);
            string fullPath = Path.Combine(specificPath, dbFileName);
        ...
        public SQLiteDataService()
        {
            ...
            {
                Console.WriteLine($"检测到数据库文件不存在于 [{dbPath}]，正在创建并初始化...");
