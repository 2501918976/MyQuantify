...
namespace MyQuantifyApp.Service
{
    ...
    internal class ActiveWindowHook
    {
            ...
            uint dwEventThread, uint dwmsEventTime)
        {
            ...
            var title = GetActiveWindowTitle(hwnd); // 推荐使用传入的 hwnd 获取标题

            // 如果窗口是前台窗口切换事件 (EVENT_SYSTEM_FOREGROUND) 
            // 或者 是标题修改事件 (EVENT_OBJECT_NAMECHANGE) 并且标题已更改
            if (!string.IsNullOrEmpty(title) && _lastWindowTitle != title)
            {
                _lastWindowTitle = title;
                // 确保调用 OnActiveWindowChanged(hwnd, title)
        ...
        private void OnActiveWindowChanged(IntPtr hwnd, string title)
        {
            ...
            handler?.Invoke(this, new ActiveWindowChangedEventArgs(title, processName, filePath)); // 错误已修复！
        }
        ...
        public void RaiseOne()
        {
            ...
            // 2. 使用 hwnd 获取窗口标题。
            // 错误 CS1501 修复：调用 GetActiveWindowTitle(hwnd) 需要传入 hwnd
            _lastWindowTitle = GetActiveWindowTitle(hwnd);

            if (!string.IsNullOrEmpty(_lastWindowTitle))
            {
                // 错误 CS7036 修复：调用 OnActiveWindowChanged 需要传入 hwnd 和 title
